...
if ($req_per == 1) {
	if (isset($_POST['add'])) {
		...
		try {
			...
			$stmt = $conn->prepare("INSERT INTO customers (customers_name,customers_phone1,customers_phone2,customers_phone3,customers_email,customers_address,customers_website,customers_purpoes,customers_renewal_date,customers_updated_date,customers_added_date) VALUES (:name,:phone_1,:phone_2,:phone_3,:customers_email,:customers_address,:customers_website,:customers_purpoes,:customers_renewal_date,:customers_updated_date,:customers_created_date)");
			$stmt->execute(['name' => $name, 'phone_1' => $phone_1, 'phone_2' => $phone_2, 'phone_3' => $phone_3, 'customers_email' => $customers_email, 'customers_address' => $customers_address, 'customers_website' => $customers_website, 'customers_purpoes' => $customers_purpoes, 'customers_renewal_date' => $customers_renewal_date, 'customers_updated_date' => $today, 'customers_created_date' => $today]);
